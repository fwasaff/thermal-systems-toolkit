...
Project: Industrial Heat Recovery Toolkit (general-purpose)
"""
...
def ntu_from_effectiveness_counterflow(epsilon: float, C_ratio: float) -> float:
    ...
    Notes
    -----
    For counterflow, solve numerically by bisection.

    effectiveness_ntu_counterflow(NTU, C_ratio) is monotonically increasing
    in NTU for fixed C_ratio (more transfer area/time can only raise the
    heat transferred, never lower it), so bisection on a bracket wide
    enough to cover epsilon is guaranteed to converge — unlike a
    fixed-step update, whose step size implicitly assumes a slope that
    is wrong precisely where the curve is flattest (C_ratio -> 1), which
    is exactly where an earlier version of this function silently
    returned NTU values off by ~9%. Bisection trades a bit of speed for
    a convergence guarantee that doesn't depend on the local slope.
    """
    if abs(C_ratio - 1.0) < 1e-6:
    ...
    # Bracket [NTU_lo, NTU_hi] such that eps(NTU_lo) <= epsilon <= eps(NTU_hi).
    # eps(0) = 0 always; grow the upper bound until it brackets epsilon
    # (eps -> 1 as NTU -> infinity for C_ratio < 1, so this always
    # terminates for any valid epsilon < 1).
    NTU_lo, NTU_hi = 0.0, 1.0
    while effectiveness_ntu_counterflow(NTU_hi, C_ratio) < epsilon:
        NTU_hi *= 2.0
        if NTU_hi > 1e6:
            break  # epsilon effectively unreachable (e.g. epsilon >= 1)

    for _ in range(100):
        NTU_mid = 0.5 * (NTU_lo + NTU_hi)
        eps_mid = effectiveness_ntu_counterflow(NTU_mid, C_ratio)
        if abs(eps_mid - epsilon) < 1e-10:
            NTU = NTU_mid
            break
        if eps_mid < epsilon:
            NTU_lo = NTU_mid
